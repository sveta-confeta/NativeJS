...
const addFriends = (students) => {
    // return students.map(m=>({...m,frends:studentsNames.filter(n=>n!==students.name)})) =1 variant
    return students.map(m => ( //student BOB
        {
            ...m, frends: students.map(s => s.name)//[ все имена]
                .filter(st => st !== m.name)
        }))//[все имена кроме BOB]
}
